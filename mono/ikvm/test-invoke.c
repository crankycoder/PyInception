...
static void access_valuetype_field (MonoObject *obj)
{
    MonoClass *klass;
...
MonoAssembly* open_assembly(MonoDomain* domain, char* file)
{
...
static void main_function (MonoDomain *domain, const char *file, int argc, char **argv)
{
    ...
    // This runs the Main() method in a particular class
    mono_jit_exec(domain, assembly, argc, argv);

    /*
     * Create an object and start accessing
     * value-type attributes, reference fields and invoking 
     * some methods
     */
    create_object(domain, mono_assembly_get_image(assembly));
}

void* initialize_vm(const char* assembly)
{
    MonoDomain *domain;
...
int shutdown_vm(MonoDomain *domain)
{
    int retval;
...
int main (int argc, char* argv[]) {
    ...
    domain = initialize_vm(file);

    main_function(domain, file, argc - 1, argv + 1);

    retval = shutdown_vm(domain);
    return retval;
}
